CREATE TABLE IF NOT EXISTS `user` (
  ...
  INDEX `ih_password` (`password`)
    USING HASH,
  INDEX `ib_password` (`password`)
    USING BTREE,
  FULLTEXT KEY `if_email_password` (`email`,`password`),
  UNIQUE KEY `iu_email_password` (`nick`),
  UNIQUE KEY `iuh_email_password` (`nick`)
    USING HASH,
  UNIQUE KEY `iub_email_password` (`nick`)
    USING BTREE
) ENGINE = InnoDB DEFAULT CHARSE T =latin1 COLLATE = latin1_general_ci;
